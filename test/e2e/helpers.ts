 ...
 *
 * By default the built standalone CLI (`bin/run.js`) runs with `CONNI_CONFIG_DIR`
 ...
 * topic-prefixed (`sdkck conni space list`), so the argv is host-agnostic —
 * and oclif's bin-scoped `SDKCK_*` dirs are redirected: config to the same
 * throwaway `conni-config.json` dir the standalone leg uses, data/cache into the
 ...
 * @param configDir The dir holding conni-config.json, from createConfigDir().
...
function hostInvocation(
  ...
  return {argv: [CLI, ...args], command: process.execPath, env: {CONNI_CONFIG_DIR: configDir}}
}

/**
 * Runs the host CLI as a real subprocess against the sandbox. The host is the
 * built standalone CLI (`bin/run.js`, which takes the `conni` topic once) unless
 * `E2E_HOST_CLI=sdkck` (see hostInvocation()).
 *
 * Non-zero exits are returned rather than thrown so tests can assert on
 * failure paths.
 *
 * @param args Command line arguments, e.g. ['conni', 'space', 'list'].
 * @param configDir Value for CONNI_CONFIG_DIR / SDKCK_CONFIG_DIR, from
 *   createConfigDir().
